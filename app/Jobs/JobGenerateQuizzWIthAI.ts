...
interface JobGenerateQuizzWIthAIPayload {
  userId: string
  data: any
}

export default class JobGenerateQuizzWIthAI extends Job {
  static get $$filepath() {
    return import.meta.url
  }

  public async handle(payload: JobGenerateQuizzWIthAIPayload) {
    ...
    if (payload.data.provider === 'openai') {
      console.log('OpenAI model is deprecated, please use Google Gemini model instead.')
      return
    }

    if (payload.data.provider === 'gemini') {
      await this.setPromptForModalGemini(payload)
    }
  }

  public async rescue(payload: JobGenerateQuizzWIthAIPayload, error: Error) {
    // save log
    await GenerateQuizHistory.updateOne(
      { _id: payload.data.historyId },
      {
  ...
  private async setPromptForModalGemini(payload: JobGenerateQuizzWIthAIPayload) {
    let dataDb = null
    try {
      const ai = new GoogleGenAI({ apiKey: 'AIzaSyDLaS8vN5gFdStPgP31jWSUj5eLQ17kW8M' })
      const prompt = await this.getPrompt(payload)
                  ...
                  items: {
                    ...
                    properties: {
                      title: {
                        type: Type.STRING,
                        description:
                          'Trong trường title chỉ (ngắn gọn, súc tich) KHÔNG quá 80 ký tự, KHÔNG thêm số thứ tự hay chữ ‘Câu ...’. Số thứ tự sẽ được quản lý riêng.”',
                      },
                      difficulty: { type: Type.STRING, enum: ['easy', 'medium', 'hard'] },
                      content: {
                        type: Type.STRING,
                        description:
                          'Nội dung câu hỏi là ở trường này, có thể bao gồm hình ảnh (base64 hoặc link), công thức toán học, biểu đồ, sơ đồ...',
                      },
                      type: {
                        type: Type.STRING,
                        enum: ['single_choice', 'multiple_choice', 'true_false', 'fill_blank'],
                        description:
                          'Loại câu hỏi, loại câu hỏi multiple_choice thì phải có nhiều đáp án đúng có ít nhất là 2 đáp án đúng',
                      },
                      answers: {
                        type: Type.ARRAY,
                        items: {
                      ...
                      points: {
                        type: Type.NUMBER,
                        description: `Điểm của câu hỏi, phụ thuộc vào độ khó, Tổng điểm của các câu hỏi phải bằng ${payload.data.totalPoints}.`,
                      },
                    },
      ...
      await QuizService.saveAiGeneratedQuiz(dataRequestSave, payload.userId as any)
      // save log
      await GenerateQuizHistory.updateOne(
        { _id: payload.data.historyId },
        {
  ...
  private async getPrompt(payload: JobGenerateQuizzWIthAIPayload) {
    return `
      ...
      8 Câu hỏi loại 'true_false' chỉ có 2 đáp án là Đúng và Sai.
      9 Các câu hỏi không được trùng lặp về nội dung hay đáp án.
      10 Mỗi câu có ít nhất 2 đáp án và có ít nhất 1 đáp đúng, cần chỉ rõ đáp án đúng.
      11 Định dạng kết quả trả về là JSON theo cấu trúc: [{ "questions": [ { "title": "Câu hỏi?", "difficulty": "easy|medium|hard", "content": "Nội dung câu hỏi", "type": "single_choice|multiple_choice|true_false|fill_blank", "answers": [ { "text": "Đáp án 1", "isCorrect": true|false, "explanation": "Giải thích đáp án" }, ... ], "points": 10 }, ... ] }]
      12 Chỉ trả về đúng định dạng JSON, không thêm bất kỳ chú thích hay văn bản nào khác ngoài JSON.`
  }
}
